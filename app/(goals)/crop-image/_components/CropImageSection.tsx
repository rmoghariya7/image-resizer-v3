...
// `ssr: false` — the tool uses browser-only APIs (Canvas, pointer/touch
// gestures via react-easy-crop, URL.createObjectURL). react-easy-crop itself
// is lazy-loaded a level deeper (see CropStage.tsx) so it isn't pulled into
// this chunk until an image is actually loaded.
const ImageCropperToolComponent = dynamic(
  () => import('@/features/image-cropper').then(m => ({ default: m.ImageCropperTool })),
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  { loading: (props: any) => (props?.error ? <ToolError /> : <ToolSkeleton />), ssr: false },
)

export function CropImageSection() {
  // Gate the dynamic import behind client mount — same React 19 hydration
  // workaround as ToolSection.tsx / VideoToAudioSection.tsx.
  ...
  return <ImageCropperToolComponent />
}
