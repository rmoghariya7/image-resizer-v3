...
// `ssr: false` — the tool uses browser-only APIs (Canvas, URL.createObjectURL).
// Same client-mount-gate + dynamic-import pattern as
// app/(goals)/crop-image/_components/CropImageSection.tsx.
const PhotoTextOverlayToolComponent = dynamic(
  () => import('@/features/photo-text-overlay').then(m => ({ default: m.PhotoTextOverlayTool })),
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  { loading: (props: any) => (props?.error ? <ToolError /> : <ToolSkeleton />), ssr: false },
)

export function AddNameAndDateSection() {
  const [mounted, setMounted] = useState(false)
  // eslint-disable-next-line react-hooks/set-state-in-effect -- intentional one-shot mount gate; see CropImageSection.tsx for the hydration rationale
  useEffect(() => { setMounted(true) }, [])

  if (!mounted) return <ToolSkeleton />

  return <PhotoTextOverlayToolComponent />
}
