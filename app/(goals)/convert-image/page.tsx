...
import { ConvertImageSection } from "./_components/ConvertImageSection";
import { DESCRIPTION, FAQS, HOW_IT_WORKS, PAGE_TITLE, SEO_TITLE } from "./content";

const CANONICAL = `${BASE_URL}/convert-image`;
...
export const metadata: Metadata = generateToolMetadata({
  ...
  canonical: CANONICAL,
  primaryQuery: "convert image format online free",
  ogBadge: "Image Converter",
  ogBadges: ["Free", "No uploads", "JPEG · PNG · WebP"],
});
...
const softwareSchema = {
  ...
  featureList: [
    ...
    "Original pixel dimensions preserved exactly",
...
const howToSchema = {
  "@context": "https://schema.org",
  "@type": "HowTo",
  name: `How to convert an image format with ${SITE_NAME}`,
        ...
        <div className="border-b border-gray-100 bg-white px-4 py-3 sm:px-6 sm:py-10">
          <div className="mx-auto max-w-3xl">
            ...
            <p className="mt-2 hidden text-base leading-relaxed text-gray-600 sm:mt-4 sm:block sm:text-lg">
              Convert between JPEG, PNG and WebP — dimensions stay exactly
              the same, only the format changes.
            </p>

            <div className="mt-4 hidden flex-wrap items-center gap-2 sm:flex">
              {["JPEG · PNG · WebP", "keeps dimensions", "no upload", "free"].map((tag) => (
                <span
                  key={tag}
