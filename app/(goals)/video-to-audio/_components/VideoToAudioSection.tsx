...
// URL.createObjectURL). The skeleton renders while the module chunk loads.
// FFmpeg itself is NOT in this chunk — it's fetched only when a video is uploaded.
const VideoToAudioToolComponent = dynamic(
  () =>
    import('@/features/video-to-audio').then(m => ({ default: m.VideoToAudioTool })),
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  { loading: (props: any) => props?.error ? <ToolError /> : <ToolSkeleton />, ssr: false },
)

export function VideoToAudioSection() {
  // Gate the dynamic import behind client mount — same React 19 hydration
  // workaround as ToolSection.tsx (see the detailed comment there): rendering
  // the skeleton until useEffect fires keeps server HTML and hydration in
  // sync, then starts the import from a committed client render.
  ...
  return <VideoToAudioToolComponent />
}
      ...
      <div className="mx-auto max-w-2xl">
        ...
        <div className="mt-3 grid grid-cols-3 gap-2 md:grid-cols-5">
          {Array.from({ length: 5 }).map((_, i) => (
            <div
              key={i}
              className={`h-16 animate-pulse rounded-xl bg-gray-200 ${i > 2 ? 'md:block' : ''}`}
            />
          ))}
        </div>
        <div className="mt-3 h-11 w-full animate-pulse rounded-lg bg-gray-200" />
      </div>
    </section>
  )
}
