 ...
 *
 * Guarantees:
 * - Runs only on mount — never on preset changes, uploads, processing, or
 *   downloads, since those update state without remounting the tool component.
 * - Runs only after hydration: the tool components are loaded via
 *   next/dynamic (ssr: false) behind a client-mount gate, so by the time this
 *   effect fires the page is fully hydrated and interactive.
