...
import type { FooterResult, OriginalImage, OutputFormat } from '../types'
...
/** Preview + dimensions + file size + format switch + Download — mirrors features/image-cropper/components/CropResultPanel.tsx. */
export function ResultPanel({ original, result, outputFormat, onFormatChange, onDownload, onMakeAnother }: Props) {
  const fileExt = result.filename.split('.').pop()?.toUpperCase() ?? 'IMAGE'

  return (
    <section aria-label="Photo with footer result" className="bg-gray-50 px-4 pt-5 pb-4 sm:px-6 md:pt-10 md:pb-6">
      <div className="mx-auto max-w-2xl">
        ...
        <figure className="overflow-hidden rounded-xl border border-indigo-200 bg-white shadow-sm ring-2 ring-indigo-100">
          <div
            className="mx-auto max-h-80 w-full bg-[repeating-conic-gradient(#f3f4f6_0%_25%,white_0%_50%)] bg-size-[16px_16px]"
            style={{ aspectRatio: `${result.width} / ${result.height}` }}
          >
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img
              src={result.objectUrl}
              alt="Photo with footer preview"
        ...
        <p className="mt-2 text-center text-xs text-gray-500">
          Original {original.width} × {original.height}px ({fmtKB(original.sizeKB)}) → With footer {result.width} × {result.height}px ({fmtKB(result.sizeKB)})
        </p>
